...
あくまでValidation用のデータセットとして使用していただけると幸いです。

## Important note
08/02/2025追記 (dd/mm/yyyy)
情報に誤りはありませんが、70Mのケースに関してはさらにCFDのセッティングをブラッシュアップすることでより現実的なシミュレーションが回せることがここ１年間の勉強でわかりました。
それにiteration 1500では十分に結果が収束しているとは言えず、ここで得た統計的な平均値には大きな誤差が含まれています。なので今後、シミュレーションをさらに回し、結果が揃い次第、解析結果をPDFにアップロードします。初期の状態でデータを比較された方もいると思うので、以下にUpdate logというセクションを設けました。Githubにあがっているデータが更新されるごとにこのセクションを書き換えていきます。

20/06/2025追記 (dd/mm/yyyy)
上記のシミュレーションをさらに回して解析するという作業が完了したので、結果をスライドという形でアップロードしました。新しく追加されたシミュレーションのIterationはすべて1500から2500に変更されており, CFDのセッティングも乱流モデルの詳細設定, およびソルバーの本稿という形でよりブラッシュアップしました。結果に関しては、前回の時点で以上の変更をアップデートしていたので差はありませんが、今回の解析ではこれらの変更の影響を定量的に評価しています。
## Update log
...
- AnalysisフォルダーにCFDのセッティングに関するパラメータスタディの結果を記載したスライドをアップロード
## Google Drive
